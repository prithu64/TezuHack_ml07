...
export function PredictionResult({
  ...
  if (!prediction)
    return (
      <div className="empty-panel">
        ...
        <p>
          Submit academic and background indicators to see the model&apos;s
          assessment.
        </p>
      </div>
    );
  return (
    <div className="result-content">
      <div
        className={`risk-badge ${prediction.risk_category.toLowerCase().replace("-", "-")}`}
      >
        <span className="status-dot" />
        {prediction.risk_category}
      </div>
      <h3>{prediction.message}</h3>
      <div className="probability-list" aria-label="Probability distribution">
        {categories.map((category) => {
          const probability = prediction.probabilities?.[category];
          return (
            <div className="probability-row" key={category}>
              <div>
                <span>{category}</span>
                <span>
              ...
              {probability !== undefined && (
                <div className="bar-track">
                  <div
                    className={`bar-fill ${category.toLowerCase().replace("-", "-")}`}
                    style={{
