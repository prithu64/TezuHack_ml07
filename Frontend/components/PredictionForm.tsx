...
function validate(values: FormValues): FormErrors {
  ...
  for (const field of numericFields) {
    ...
    if (
      field === "attendance" ||
      field === "assignments_completed_pct" ||
      field === "previous_grade"
    ) {
      if (value < 0 || value > 100)
        errors[field] = "Enter a value from 0 to 100.";
    }
    if (field === "study_hours" && value < 0)
      errors[field] = "Study hours cannot be negative.";
    if (
      field === "past_failures" &&
...
export function PredictionForm({ onPrediction }: PredictionFormProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSubmitError(
        "Unable to connect to the prediction service. Please check that the FastAPI backend is running and try again.",
      );
    } finally {
  ...
  return (
    <form className="prediction-form" onSubmit={handleSubmit} noValidate>
      <div className="form-section">
        <div className="section-heading">
          ...
          <div>
            ...
            <p>Core indicators from the student record.</p>
          </div>
        </div>
        <div className="form-grid">
          ...
          <Field
            label="Past failures"
            name="past_failures"
            value={values.past_failures}
            onChange={updateField}
            error={errors.past_failures}
            type="number"
            min="0"
            max="5"
            step="1"
          ...
          <Field
            ...
            name="assignments_completed_pct"
            value={values.assignments_completed_pct}
            onChange={updateField}
            error={errors.assignments_completed_pct}
            type="number"
            min="0"
            max="100"
            suffix="%"
      ...
      <div className="form-section">
        <div className="section-heading">
          ...
          <div>
            ...
            <p>Contextual indicators used by the model.</p>
          </div>
        </div>
        <div className="form-grid background-grid">
          <SelectField
            label="Parental education"
            name="parental_education"
            value={values.parental_education}
            onChange={updateField}
            error={errors.parental_education}
            options={["Higher", "Secondary", "Primary", "none"]}
          />
          <SelectField
            label="Family income"
            name="family_income"
      ...
      <div className="form-actions">
        <p>
          Predictions support review and intervention planning; they do not
        ...
        >
          {isSubmitting ? (
            <>
              <span className="button-spinner" aria-hidden="true" /> Analyzing
              indicators...
            </>
          ) : (
            <>
              Run risk assessment <span aria-hidden="true">-&gt;</span>
            </>
          )}
        </button>
      </div>
...
interface FieldProps {
  ...
  suffix?: string;
  error?: string;
  onChange: (name: keyof PredictionRequest, value: string) => void;
}
function Field({
  label,
  name,
