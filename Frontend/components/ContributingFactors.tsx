export function ContributingFactors({ factors }: { factors?: string[] }) {
  ...
  return <div className="factor-list">{factors.map((factor, index) => <article className="factor-row" key={`${factor}-${index}`}><div className="factor-index">0{index + 1}</div><h3>{factor}</h3></article>)}</div>;
}
