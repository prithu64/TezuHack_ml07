...
export function SummaryCards({ history }: { history: HistoryRecord[] }) {
  ...
  const atRisk = history.filter(
    (record) => record.risk_category === "At-Risk",
  ).length;
  ...
  const cards = [
    ...
    [
      "At-Risk students",
      total ? atRisk.toString() : "--",
      "May need additional support",
    ],
    [
      "High-Risk students",
      total ? highRisk.toString() : "--",
      "Requires closer attention",
    ],
    [
      "Average confidence",
      average === null ? "--" : `${Math.round(average * 100)}%`,
      "Across recorded predictions",
    ],
  ];

  return (
    <section className="summary-grid" aria-label="Prediction summary">
